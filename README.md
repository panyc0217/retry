...
设置延迟策略，用于计算下次重试前的等待时间。

内置延迟策略：
1. `FixedDelay(delay time.Duration)`：固定时间间隔策略，每次重试前等待相同的时间。
2. `ExponentialDelay(baseDelay, maxDelay time.Duration)`：指数时间间隔退避策略，延迟时间以2的指数倍增长。
